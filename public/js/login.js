(function() {
  ...
  formEmail.addEventListener("submit", function(e) {
    ...
    fetch(CHECK_URL, {
      ...
      if (data.status === "password" || data.status === "added") {
        ...
        var greeting2 = document.getElementById("password-greeting");
        if (data.status === "added") {
          greeting2.textContent = "Welcome, " + (data.name || email) + "! We've added you to this program. Please enter your password to continue.";
        } else {
          greeting2.textContent = "Welcome back" + (data.name ? ", " + data.name : "") + "! Please enter your password.";
        }
  ...
  function resetEmailStep() {
    ...
    setError("email-error", "");
    document.querySelectorAll('#step-password p[style*="color:#c0392b"]').forEach(function(p) {
      p.remove();
    });
