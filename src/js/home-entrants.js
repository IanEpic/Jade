// home-entrants.js — inline entrant new/edit form behaviour
...
function setSelectVal(el, val) {
    if (!el) return;
    // Try exact match first; fall back to 'a' (please select)
    var found = false;
    for (var i = 0; i < el.options.length; i++) {
        if (el.options[i].value === String(val)) { el.selectedIndex = i; found = true; break; }
    }
    if (!found) el.value = 'a';
}
...
// Edit button — populate form from data attributes on the row
...
// Initialise address field visibility on page load
checkAddressFields();
